...
if (!handle_collision_x(obj_player) && !handle_collision_x(abstract_obj_enemy) && !stunned) {
	show_debug_message("moving");
	x += cur_v.x;
}

if (!handle_collision_y(obj_player) && !handle_collision_y(abstract_obj_enemy) && !stunned) {
	show_debug_message("moving");
	y += cur_v.y;
}
